...
class PickTools():
    ...
    def make_grasps_msgs(self, object_name, grasp_pose, grasp_quality=1.0):
        '''
        ...
        g.grasp_quality = grasp_quality

        # The internal posture of the hand for the grasp
        # positions and efforts are used
        g.grasp_posture = self.gripper_open_trajectory

        # The approach direction to take before picking an object
        g.pre_grasp_approach = self.make_gripper_translation_msg(self.arm.get_end_effector_link(), min_dist=0.08, desired=0.25, axis=[1.0, 0.0, 0.0])
        ...
        g.pre_grasp_posture = self.gripper_close_trajectory

        # The retreat direction to take after a grasp has been completed (object is attached)
        g.post_grasp_retreat = self.make_gripper_translation_msg('world', min_dist=0.1, desired=0.15, axis=[0.0, 0.0, 1.0])
    ...
    def pick_object(self, object_name):
        ...
        grasp_pose.pose.position.x =     0.017
        grasp_pose.pose.position.y =     0.674
        grasp_pose.pose.position.z =     1.156
        grasp_pose.pose.orientation.x =  0.352
        grasp_pose.pose.orientation.y = -0.512
        grasp_pose.pose.orientation.z =  0.392
        grasp_pose.pose.orientation.w =  0.679
